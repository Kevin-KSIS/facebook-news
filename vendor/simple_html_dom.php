...
class simple_html_dom_node
{
	...
	function text()
	{
		...
		if (strcasecmp($this->tag, 'scripts') === 0) { return ''; }
		if (strcasecmp($this->tag, 'styles') === 0) { return ''; }
	...
	function get_display_size()
	{
		...
		// Now look for an inline styles.
		if (isset($this->attr['styles'])) {
			...
			// If there is a width in the styles attributes:
...
class simple_html_dom
{
		...
		$options = 0)
	{
		...
		// Script tags removal now preceeds styles tag removal.
		// strip out <scripts> tags
		$this->remove_noise("'<\s*scripts[^>]*[^/]>(.*?)<\s*/\s*scripts\s*>'is");
		$this->remove_noise("'<\s*scripts\s*>(.*?)<\s*/\s*scripts\s*>'is");
		...
		$this->remove_noise("'<\s*styles[^>]*[^/]>(.*?)<\s*/\s*styles\s*>'is");
		$this->remove_noise("'<\s*styles\s*>(.*?)<\s*/\s*styles\s*>'is");
